...
import { callUpstreamReact } from './upstream';
...
export async function enqueue({ requestId, identifier, plan, url, reaction, coinCharged }) {
...
export async function tryProcessNext() {
  ...
  try {
    ...
    if (success) {
      await taskRef.update({ status: 'success', result: upstream.body ?? null, updatedAt: FieldValue.serverTimestamp() });
      ...
      if (task.coinCharged) await refundCoin(task.identifier).catch(() => {});
      ...
      await logEvent('upstream_error', 'Upstream reaction call failed', { requestId: task.id, httpStatus: upstream.status });
    }
    return { processed: true, requestId: task.id };
  } catch (err) {
    if (candidate.coinCharged) await refundCoin(candidate.identifier).catch(() => {});
