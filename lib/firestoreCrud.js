...
// (devKeys for API access, redeemCodes, promotions). Reduces boilerplate across
  ...
  return withAdminAuth(async (req, res) => {
    ...
    try {
      ...
      if (req.method === 'POST') {
        ...
        // idPrefix can be a static string, or a function of the submitted
        // data - used by redeemCodes so VIP/DEV-type codes are prefixed
        // "VIP-"/"DEV-" instead of always "RDM-".
        const prefix = typeof idPrefix === 'function' ? idPrefix(data) : idPrefix;
        const id = generateId(prefix, idLength);
